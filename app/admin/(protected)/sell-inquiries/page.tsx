...
import { getSellInquiries, updateSellInquiryStatus } from "@/lib/sellInquiries";
import { SellInquiry } from "@/lib/types";
...
export default function SellInquiriesPage() {
  ...
  const handleStatus = async (id: string, status: SellInquiry["status"]) => {
    setUpdating(id);
    try {
      await updateSellInquiryStatus(id, status);
  ...
  const tabCount = (t: Filter) =>
    (t === "all" ? items : items.filter((i) => i.status === t)).length;
  ...
  return (
    <div>
      <div className="mb-8">
        <p className="text-[10px] font-bold tracking-[0.4em] uppercase text-[#cc1111] mb-1">Sell a Car</p>
        <h1 className="font-display text-4xl text-gray-900 tracking-wide">Sell Inquiries</h1>
      </div>
      ...
      <div className="bg-white border border-gray-200 p-4 mb-5 flex flex-wrap items-center gap-3">
        ...
        {search && (
          <button
            onClick={() => setSearch("")}
            className="flex items-center gap-1.5 text-[10px] font-bold tracking-[0.2em] uppercase text-gray-400 hover:text-[#cc1111]"
          >
            <X size={12} /> Clear
          </button>
        )}
      ...
      <div className="bg-white border border-gray-200">
        {loading ? (
          ...
          <div className="py-16 text-center text-gray-400 text-sm">No sell inquiries found.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  {["Name", "Contact", "Vehicle", "Mileage", "Condition", "Asking Price", "Date", "Status", "Action"].map((h) => (
                    <th key={h} className="px-4 py-3 text-left text-[9px] font-bold tracking-[0.3em] uppercase text-gray-400">
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {filtered.map((item) => (
                  <>
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-sm text-gray-500 whitespace-nowrap">{item.mileage} km</td>
                      <td className="px-4 py-4 text-sm text-gray-500">{item.condition}</td>
                      <td className="px-4 py-4 text-sm font-semibold text-gray-900 whitespace-nowrap">₱ {item.askingPrice}</td>
                    ...
                    {/* Expanded notes row */}
                    {expanded === item.id && (
                      <tr key={`${item.id}-expanded`} className="bg-gray-50 border-b border-gray-100">
                        <td colSpan={9} className="px-4 py-4">
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs text-gray-600 mb-3">
                            <div><span className="font-bold uppercase tracking-wider text-gray-400 block mb-0.5">Transmission</span>{item.transmission}</div>
                            <div><span className="font-bold uppercase tracking-wider text-gray-400 block mb-0.5">Condition</span>{item.condition}</div>
                            <div><span className="font-bold uppercase tracking-wider text-gray-400 block mb-0.5">Mileage</span>{item.mileage} km</div>
                            <div><span className="font-bold uppercase tracking-wider text-gray-400 block mb-0.5">Asking Price</span>₱ {item.askingPrice}</div>
                          </div>
                          {item.notes && (
                            <div>
                              <span className="font-bold uppercase tracking-wider text-gray-400 text-xs block mb-1">Notes</span>
